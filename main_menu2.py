...
def get_callable_functions(file_path):
...
def main_menu():
    ...
    if str(file_folder_input).lower() != "folder":
        ...
        # Check if the file_name_input requires custom input
        if file_name_input in file_custom_input:
            custom_input = input("Enter custom input for special_script.py: ")
            # Execute special_script.py with custom input
            subprocess.run(["python", file_path_input, custom_input])
        else:
