...
#                     except:
#                         WARN("Couldn't figure out location: {}".format(e))
#
...
#             # add a new heading with the chapter title
...
class KindleNotes:
                ...
                try:
                    ...
                    if last_note_type == 'Note':
                        try:
                            ...
                            logging.warning(f"Exception getting last-inserted note: {e}")
                            wip_note = None
                    else:
                        wip_note = None
                    ...
                    logging.warning(f"Couldn't figure out location from {source}: {e}")
    ...
    def output_md(self, args):
        ...
        if args.clipboard:
            pyperclip.copy(md)
            logging.info("Copied the output to clipboard")
        else:
            output_path = Path(args.output)
            if not output_path.exists() or args.override:
                with output_path.open('w', encoding='utf8') as fp:
                    fp.write(md)
                logging.info(f"Wrote the output to {args.output}")
            else:
                logging.info("Could not save .md file, because it already exists. Use --override flag.")


def parse_command_line_args():
    description = "Convert an HTML file of book notes exported from an Amazon Kindle to a Markdown document "
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('input', type=argparse.FileType('r'), help='Input HTML file')
    parser.add_argument('-nl', '--no-location', dest='location', action='store_false', default=True,
                        help='Whether to skip export of location of notes/highlights')
    parser.add_argument('-c', '--clipboard', action='store_true', help='Use to export .md directly to the clipboard instead of file')
    parser.add_argument('-y', '--override', action='store_true', default=False, help='Whether to override .md file in case if one already exists')
    parser.add_argument('-o', '--output', default='', help='A file to which save the Markdown document')
    args = parser.parse_args()
    args.output = os.path.splitext(args.input.name)[0] + '.md'
    return args
...
if __name__ == '__main__':
    try:
        args = parse_command_line_args()
        notes = KindleNotes()
        notes.parse_file(args.input.name)
        notes.output_md(args)
        input("Press any key to exit")
